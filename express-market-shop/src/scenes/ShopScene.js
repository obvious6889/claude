...
const ENTER_DOOR   = { x: 340, y: 655 }; // покупці входять
const EXIT_DOOR    = { x: 690, y: 655 }; // покупці виходять
...
const SPAWN_INTERVAL = (11 * 60 * 1000) / 100;

// Realistic customer looks
...
export default class ShopScene extends Phaser.Scene {
  ...
  _drawHuman(g, v) {
    ...
    // Neck
    g.fillStyle(skin); g.fillRect(-4, -28, 8, 10);
    // Head
    g.fillStyle(skin); g.fillCircle(0, -38, 12);
    // Hair
  ...
  _drawStore() {
    ...
    // ВХІД door (customers enter, bottom-left area)
    ...
    // ВИХІД door (customers exit, bottom-right area)
  ...
  _spawnCustomer() {
    if (this.spawnedCount >= 100) return;
